  ...
  function () {
    ...
    context(`${contract}`, function () {
      beforeEach(async function () {
        this.erc721l = await deployContract(contract, constructorArgs);
        this.receiver = await deployContract('ERC721ReceiverMock', [RECEIVER_MAGIC_VALUE, this.erc721l.address]);
        this.startTokenId = this.erc721l.startTokenId ? (await this.erc721l.startTokenId()).toNumber() : 0;
      ...
      describe('EIP-165 support', async function () {
        it('supports ERC165', async function () {
          expect(await this.erc721l.supportsInterface('0x01ffc9a7')).to.eq(true);
        });

        it('supports IERC721', async function () {
          expect(await this.erc721l.supportsInterface('0x80ac58cd')).to.eq(true);
        });

        it('supports ERC721Metadata', async function () {
          expect(await this.erc721l.supportsInterface('0x5b5e139f')).to.eq(true);
        });

        it('does not support ERC721Enumerable', async function () {
          expect(await this.erc721l.supportsInterface('0x780e9d63')).to.eq(false);
        });

        it('does not support random interface', async function () {
          expect(await this.erc721l.supportsInterface('0x00000042')).to.eq(false);
        });
      });

      describe('ERC721Metadata support', async function () {
        it('name', async function () {
          expect(await this.erc721l.name()).to.eq(constructorArgs[0]);
        });

        it('symbol', async function () {
          expect(await this.erc721l.symbol()).to.eq(constructorArgs[1]);
        });

        describe('baseURI', async function () {
          it('sends an empty URI by default', async function () {
            expect(await this.erc721l.baseURI()).to.eq('');
          });
        });
      });
      ...
      context('with minted tokens', async function () {
        beforeEach(async function () {
          ...
          await this.erc721l['safeMint(address,uint256)'](addr1.address, this.addr1.expected.mintCount);
          await this.erc721l['safeMint(address,uint256)'](addr2.address, this.addr2.expected.mintCount);
          await this.erc721l['safeMint(address,uint256)'](addr3.address, this.addr3.expected.mintCount);
        });

        describe('tokenURI (ERC721Metadata)', async function () {
          describe('tokenURI', async function () {
            it('sends an empty uri by default', async function () {
              expect(await this.erc721l.tokenURI(offsetted(0))).to.eq('');
            });

            it('reverts when tokenId does not exist', async function () {
              await expect(this.erc721l.tokenURI(offsetted(this.expectedMintCount))).to.be.revertedWith(
        ...
        describe('exists', async function () {
          it('verifies valid tokens', async function () {
            for (let tokenId = offsetted(0); tokenId < offsetted(this.expectedMintCount); tokenId++) {
              const exists = await this.erc721l.exists(tokenId);
              expect(exists).to.be.true;
            }
          });

          it('verifies invalid tokens', async function () {
            expect(await this.erc721l.exists(offsetted(this.expectedMintCount))).to.be.false;
          });
        });

        describe('balanceOf', async function () {
          it('returns the amount for a given address', async function () {
            expect(await this.erc721l.balanceOf(this.owner.address)).to.equal('0');
            expect(await this.erc721l.balanceOf(this.addr1.address)).to.equal(this.addr1.expected.mintCount);
            expect(await this.erc721l.balanceOf(this.addr2.address)).to.equal(this.addr2.expected.mintCount);
            expect(await this.erc721l.balanceOf(this.addr3.address)).to.equal(this.addr3.expected.mintCount);
          });
          ...
          it('returns correct amount with transferred tokens', async function () {
            ...
            // sanity check
            expect(await this.erc721l.ownerOf(tokenIdToTransfer)).to.equal(this.addr3.address);

            expect(await this.erc721l.balanceOf(this.addr2.address)).to.equal(this.addr2.expected.mintCount - 1);
            expect(await this.erc721l.balanceOf(this.addr3.address)).to.equal(this.addr3.expected.mintCount + 1);
          });

          it('throws an exception for the 0 address', async function () {
            await expect(this.erc721l.balanceOf(ZERO_ADDRESS)).to.be.revertedWith('BalanceQueryForZeroAddress');
          });
        });

        describe('_numberMinted', async function () {
          it('returns the amount for a given address', async function () {
            expect(await this.erc721l.numberMinted(this.owner.address)).to.equal('0');
            expect(await this.erc721l.numberMinted(this.addr1.address)).to.equal(this.addr1.expected.mintCount);
            expect(await this.erc721l.numberMinted(this.addr2.address)).to.equal(this.addr2.expected.mintCount);
            expect(await this.erc721l.numberMinted(this.addr3.address)).to.equal(this.addr3.expected.mintCount);
          });
        ...
        describe('aux', async function () {
          it('get and set works correctly', async function () {
            ...
            expect(await this.erc721l.getAux(this.owner.address)).to.equal('0');
            await this.erc721l.setAux(this.owner.address, uint64Max);
            expect(await this.erc721l.getAux(this.owner.address)).to.equal(uint64Max);

            expect(await this.erc721l.getAux(this.addr1.address)).to.equal('0');
            await this.erc721l.setAux(this.addr1.address, '1');
            expect(await this.erc721l.getAux(this.addr1.address)).to.equal('1');

            await this.erc721l.setAux(this.addr3.address, '5');
            expect(await this.erc721l.getAux(this.addr3.address)).to.equal('5');
          ...
          it('returns the right owner', async function () {
            for (const minter of [this.addr1, this.addr2, this.addr3]) {
              for (const tokenId of minter.expected.tokens) {
                expect(await this.erc721l.ownerOf(tokenId)).to.equal(minter.address);
              }
            }
          });

          it('reverts for an invalid token', async function () {
            await expect(this.erc721l.ownerOf(10)).to.be.revertedWith('OwnerQueryForNonexistentToken');
          });
        });
        ...
        describe('approve', async function () {
          ...
          it('sets approval for the target address', async function () {
            await this.erc721l.connect(this.addr1).approve(this.addr2.address, this.tokenId);
            const approval = await this.erc721l.getApproved(this.tokenId);
            expect(approval).to.equal(this.addr2.address);
          });

          it('set approval for the target address on behalf of the owner', async function () {
            await this.erc721l.connect(this.addr1).setApprovalForAll(this.addr2.address, true);
            await this.erc721l.connect(this.addr2).approve(this.addr3.address, this.tokenId);
            const approval = await this.erc721l.getApproved(this.tokenId);
            expect(approval).to.equal(this.addr3.address);
          });

          it('rejects an unapproved caller', async function () {
            await expect(this.erc721l.approve(this.addr2.address, this.tokenId)).to.be.revertedWith(
              'ApprovalCallerNotOwnerNorApproved'
            );
          });

          it('does not get approved for invalid tokens', async function () {
            await expect(this.erc721l.getApproved(10)).to.be.revertedWith('ApprovalQueryForNonexistentToken');
          });

          it('approval allows token transfer', async function () {
            await expect(
              this.erc721l.connect(this.addr3).transferFrom(this.addr1.address, this.addr3.address, this.tokenId)
            ).to.be.revertedWith('TransferCallerNotOwnerNorApproved');
            await this.erc721l.connect(this.addr1).approve(this.addr3.address, this.tokenId);
            await this.erc721l.connect(this.addr3).transferFrom(this.addr1.address, this.addr3.address, this.tokenId);
            await expect(
              this.erc721l.connect(this.addr1).transferFrom(this.addr3.address, this.addr1.address, this.tokenId)
            ).to.be.revertedWith('TransferCallerNotOwnerNorApproved');
          });

          it('token owner can approve self as operator', async function () {
            expect(await this.erc721l.getApproved(this.tokenId)).to.not.equal(this.addr1.address);
            await expect(this.erc721l.connect(this.addr1).approve(this.addr1.address, this.tokenId)
            ).to.not.be.reverted;
            expect(await this.erc721l.getApproved(this.tokenId)).to.equal(this.addr1.address);
          });

          it('self-approval is cleared on token transfer', async function () {
            await this.erc721l.connect(this.addr1).approve(this.addr1.address, this.tokenId); 
            expect(await this.erc721l.getApproved(this.tokenId)).to.equal(this.addr1.address);

            await this.erc721l.connect(this.addr1).transferFrom(this.addr1.address, this.addr2.address, this.tokenId);
            expect(await this.erc721l.getApproved(this.tokenId)).to.not.equal(this.addr1.address);
          });

          it('direct approve works', async function () {
            expect(await this.erc721l.getApproved(this.tokenId)).to.not.equal(this.addr1.address);
            await this.erc721l.connect(this.addr2).directApprove(this.addr1.address, this.tokenId); 
            expect(await this.erc721l.getApproved(this.tokenId)).to.equal(this.addr1.address);
          });
        });
        ...
        describe('setApprovalForAll', async function () {
          it('sets approval for all properly', async function () {
            const approvalTx = await this.erc721l.setApprovalForAll(this.addr1.address, true);
            await expect(approvalTx)
              .to.emit(this.erc721l, 'ApprovalForAll')
              .withArgs(this.owner.address, this.addr1.address, true);
            expect(await this.erc721l.isApprovedForAll(this.owner.address, this.addr1.address)).to.be.true;
          });

          it('caller can approve all with self as operator', async function () {
            expect(
              await this.erc721l.connect(this.addr1).isApprovedForAll(this.addr1.address, this.addr1.address)
            ).to.be.false;
            await expect(
              this.erc721l.connect(this.addr1).setApprovalForAll(this.addr1.address, true)
            ).to.not.be.reverted;
            expect(
              await this.erc721l.connect(this.addr1).isApprovedForAll(this.addr1.address, this.addr1.address)
            ).to.be.true;
        ...
        context('test transfer functionality', function () {
          const testSuccessfulTransfer = function (transferFn, transferToContract = true) {
            beforeEach(async function () {
              ...
              await this.erc721l.connect(sender).approve(this.to.address, this.tokenId);

              const ownershipBefore = await this.erc721l.getOwnershipAt(this.tokenId);
              ...
              // prettier-ignore
              this.transferTx = await this.erc721l
                .connect(sender)[transferFn](this.from, this.to.address, this.tokenId);

              const ownershipAfter = await this.erc721l.getOwnershipAt(this.tokenId);
              this.timestampAfter = parseInt(ownershipAfter.startTimestamp);
            });

            it('transfers the ownership of the given token ID to the given address', async function () {
              expect(await this.erc721l.ownerOf(this.tokenId)).to.be.equal(this.to.address);
            });

            it('emits a Transfer event', async function () {
              await expect(this.transferTx)
                .to.emit(this.erc721l, 'Transfer')
                .withArgs(this.from, this.to.address, this.tokenId);
            });

            it('clears the approval for the token ID', async function () {
              expect(await this.erc721l.getApproved(this.tokenId)).to.be.equal(ZERO_ADDRESS);
            });

            it('adjusts owners balances', async function () {
              expect(await this.erc721l.balanceOf(this.from)).to.be.equal(1);
            });
          ...
          const testUnsuccessfulTransfer = function (transferFn) {
            ...
            it('rejects unapproved transfer', async function () {
              await expect(
                this.erc721l.connect(this.sender)[transferFn](this.addr2.address, this.sender.address, this.tokenId)
              ).to.be.revertedWith('TransferCallerNotOwnerNorApproved');
            });

            it('rejects transfer from incorrect owner', async function () {
              await this.erc721l.connect(this.addr2).setApprovalForAll(this.sender.address, true);
              await expect(
                this.erc721l.connect(this.sender)[transferFn](this.addr3.address, this.sender.address, this.tokenId)
            ...
            it('rejects transfer to zero address', async function () {
              await this.erc721l.connect(this.addr2).setApprovalForAll(this.sender.address, true);
              await expect(
                this.erc721l.connect(this.sender)[transferFn](this.addr2.address, ZERO_ADDRESS, this.tokenId)
          ...
          context('unsuccessful transfers', function () {
            ...
            describe('safeTransferFrom', function () {
              ...
              it('reverts for non-receivers', async function () {
                const nonReceiver = this.erc721l;
                // prettier-ignore
                await expect(
                  this.erc721l.connect(this.addr1)['safeTransferFrom(address,address,uint256)'](
                      this.addr1.address,
                      nonReceiver.address,
                      offsetted(0)
                    )
              ...
              it('reverts when the receiver reverted', async function () {
                // prettier-ignore
                await expect(
                  this.erc721l.connect(this.addr1)['safeTransferFrom(address,address,uint256,bytes)'](
                      this.addr1.address,
        ...
        describe('_burn', async function () {
          ...
          it('can burn if approvalCheck is false', async function () {
            expect(await this.erc721l.exists(this.tokenIdToBurn)).to.be.true;
            await this.erc721l.connect(this.addr2)['burn(uint256,bool)'](this.tokenIdToBurn, false);
            expect(await this.erc721l.exists(this.tokenIdToBurn)).to.be.false;
          });

          it('revert if approvalCheck is true', async function () {
            await expect(
              this.erc721l.connect(this.addr2)['burn(uint256,bool)'](this.tokenIdToBurn, true)
            ).to.be.revertedWith('TransferCallerNotOwnerNorApproved');
          });

          it('can burn without approvalCheck parameter', async function () {
            expect(await this.erc721l.exists(this.tokenIdToBurn)).to.be.true;
            await this.erc721l.connect(this.addr2)['burn(uint256)'](this.tokenIdToBurn);
            expect(await this.erc721l.exists(this.tokenIdToBurn)).to.be.false;
          });
        ...
        describe('_initializeOwnershipAt', async function () {
          it('successfuly sets ownership of empty slot', async function () {
            ...
            const ownership1 = await this.erc721l.getOwnershipAt(lastTokenId);
            expect(ownership1[0]).to.equal(ZERO_ADDRESS);
            await this.erc721l.initializeOwnershipAt(lastTokenId);
            const ownership2 = await this.erc721l.getOwnershipAt(lastTokenId);
            expect(ownership2[0]).to.equal(this.addr3.address);
          });

          it("doesn't set ownership if it's already setted", async function () {
            const lastTokenId = this.addr3.expected.tokens[2];
            expect(await this.erc721l.ownerOf(lastTokenId)).to.be.equal(this.addr3.address);
            const tx1 = await this.erc721l.initializeOwnershipAt(lastTokenId);
            expect(await this.erc721l.ownerOf(lastTokenId)).to.be.equal(this.addr3.address);
            const tx2 = await this.erc721l.initializeOwnershipAt(lastTokenId);
      ...
      context('test mint functionality', function () {
        ...
        const testSuccessfulMint = function (safe, quantity, mintForContract = true) {
          beforeEach(async function () {
            ...
            this.balanceBefore = (await this.erc721l.balanceOf(this.minter.address)).toNumber();

            this.timestampToMine = (await getBlockTimestamp()) + 12345;
            await mineBlockTimestamp(this.timestampToMine);
            this.timestampMined = await getBlockTimestamp();

            this.mintTx = await this.erc721l[mintFn](this.minter.address, quantity);
          });

          it('changes ownership', async function () {
            for (let tokenId = offsetted(0); tokenId < offsetted(quantity); tokenId++) {
              expect(await this.erc721l.ownerOf(tokenId)).to.equal(this.minter.address);
            }
          });
          ...
          it('adjusts owners balances', async function () {
            expect(await this.erc721l.balanceOf(this.minter.address)).to.be.equal(this.balanceBefore + quantity);
          });

          it('adjusts OwnershipAt and OwnershipOf', async function () {
            const ownership = await this.erc721l.getOwnershipAt(offsetted(0));
            ...
            for (let tokenId = offsetted(0); tokenId < offsetted(quantity); tokenId++) {
              const ownership = await this.erc721l.getOwnershipOf(tokenId);
        ...
        const testUnsuccessfulMint = function (safe) {
          ...
          it('rejects mints to the zero address', async function () {
            await expect(this.erc721l[this.mintFn](ZERO_ADDRESS, 1)).to.be.revertedWith('MintToZeroAddress');
          });

          it('requires quantity to be greater than 0', async function () {
            await expect(this.erc721l[this.mintFn](this.owner.address, 0)).to.be.revertedWith('MintZeroQuantity');
          });
        };
          ...
          context('mint', function () {
            context('for contract', function () {
              ...
              it('does not revert for non-receivers', async function () {
                const nonReceiver = this.erc721l;
                await this.erc721l.mint(nonReceiver.address, 1);
                expect(await this.erc721l.ownerOf(offsetted(0))).to.equal(nonReceiver.address);
              });
            });
          ...
          context('safeMint', function () {
            context('for contract', function () {
              ...
              it('validates ERC721Received with custom _data', async function () {
                ...
                const tx = await this.erc721l['safeMint(address,uint256,bytes)'](this.receiver.address, 1, customData);
                await expect(tx)
                  .to.emit(this.receiver, 'Received')
        ...
        context('unsuccessful mints', function () {
          ...
          context('safeMint', function () {
            ...
            it('reverts for non-receivers', async function () {
              const nonReceiver = this.erc721l;
              await expect(this.erc721l['safeMint(address,uint256)'](nonReceiver.address, 1)).to.be.revertedWith(
            ...
            it('reverts when the receiver reverted', async function () {
              await expect(
                this.erc721l['safeMint(address,uint256,bytes)'](this.receiver.address, 1, '0x01')
              ).to.be.revertedWith('reverted in the receiver contract!');
            });

            it('reverts if the receiver returns the wrong value', async function () {
              await expect(
                this.erc721l['safeMint(address,uint256,bytes)'](this.receiver.address, 1, '0x02')
              ).to.be.revertedWith('TransferToNonERC721ReceiverImplementer');
            });

            it('reverts with reentrant call', async function () {
              await expect(
                this.erc721l['safeMint(address,uint256,bytes)'](this.receiver.address, 1, '0x03')
              ).to.be.reverted;
            });
          });
        });
      ...
      context('_toString', async function () {
        it('returns correct value', async function () {
          expect(await this.erc721l['toString(uint256)']('0')).to.eq('0');
          expect(await this.erc721l['toString(uint256)']('1')).to.eq('1');
          expect(await this.erc721l['toString(uint256)']('2')).to.eq('2');
          const uint256Max = BigNumber.from(2).pow(256).sub(1).toString();
          expect(await this.erc721l['toString(uint256)'](uint256Max)).to.eq(uint256Max);
        });
      });
    });
  };
